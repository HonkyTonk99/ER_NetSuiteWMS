...
| **Q-33** | ⚠️ *(New, from D-14 / F-28 — **blocks Phase 6 scope**.)* D-14 routes inter-location movement through a **NetSuite Transfer Order**, but **TO outbound (pick/stage/ship out of the source) is scoped nowhere** — outbound is Sales-Order-only across Phases 6–8. The mandated path cannot be executed; stock leaves a warehouse with no WMS pick. | The wave/pick engine cannot be designed until this is answered, and source-location bin state accuracy depends on it. | **Phase 6 scope** | **(a)** TO outbound in scope — extend wave/pick/allocation to `transferorder` (bin state stays accurate; largest build); **(b)** TO outbound in NetSuite UI outside the WMS (bin state goes stale until destination receipt; smallest build, largest risk); **(c)** inter-location movement out of scope for v1 (accurate, but constrains operations). | | |
