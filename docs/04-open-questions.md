...
| ~~Q-16~~ | **CLOSED 2026-08-09.** Bin types are **UNIT, BULK, STAGE, RECEIVING, QUALITY, RETURN, DEFECT.** New policy attribute **`availableForFulfilment`** — TRUE for UNIT and BULK only; stock in any other type is physically present but must be moved into UNIT/BULK before it can be picked. Enforced in T-7.1 (allocation), T-5.2 (replenishment), T-5.4 (putaway); **not** filtered from reconciliation (T-8.3). Surfaces new finding **F-26** (NetSuite over-commits non-fulfillable stock). Seeds §3.9b policy table and types the migrated bins (T-0.4 case b, Phase 10). | — | — | — | TK | **Closed** |
