...
| **Q-16** | *(New, from F-18.)* Which physical areas need bin types beyond UNIT/BULK — staging, receiving dock, QC hold, returns, cross-dock? | AD-14 makes these configuration, but the list must be known to seed the policy table and to type existing bins during Phase 10. | Phase 1, Phase 10 | Walk the floor with the warehouse manager and enumerate. | | |
