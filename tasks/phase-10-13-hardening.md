...
- **`persist()`-denied degradation (D-13 residual)** — with storage persistence denied, the app still
  runs, warns, and relies on the unsynced-count / hard-block safety net; the accepted residual is
  exercised, not assumed away.
- **Service-worker shell reload (D-13)** — with the network offline, reloading the app is served from
  the **service-worker-cached shell** and operates against IndexedDB; the master-data warm is **not**
  served from the service worker (shell ≠ data).
...
- [ ] GIVEN `navigator.storage.persist()` is denied, THEN the app runs, warns storage is best-effort, and the depth/age hard-block still protects against unbounded loss (D-13 residual).
- [ ] GIVEN the network is offline, WHEN the app is reloaded, THEN the service-worker shell restores it and it runs against IndexedDB, with the master-data warm not served from the service worker (D-13).
